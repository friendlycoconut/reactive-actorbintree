...
class BinaryTreeNode(val elem: Int, initiallyRemoved: Boolean) extends Actor {
  ...
  val normal: Receive = { case msg @ Contains(req, id, e)=>
      if (e==elem)
  ...
  case msg @ Insert (req, id, e)=>
    if (e==elem){
      ...
      subtrees += (if (e<elem) Left else  Right) -> context.actorOf(props(e, false))
      req ! OperationFinished(id)
    }
  case CopyTo(newRoot)=>
    ...
    if (children.isEmpty && removed)
      ...
      for {c<- children} c ! CopyTo(newRoot)
  ...
  def copying(expected: Set[ActorRef], insertConfirmed: Boolean): Receive = {
    ...
    case OperationFinished(id)  if id == elem =>
      if (expected.isEmpty)
        context.parent ! CopyFinished
      else
